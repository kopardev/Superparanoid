...
my $usage =" Usage: inparanoid.pl <runblast 1 or 0> <runinparanoid 1 or 0> <FASTAFILE with sequences of species A> <FASTAFILE with sequences of species B> [FASTAFILE with sequences of species C]
...
$run_blast = shift;  # Set to 1 if you don't have the 4 BLAST output files        #
...
$blastall = "$cfgHash{blastall} -a $cfgHash{ncpus}";  #Add -aN to use N processors
$formatdb = "$cfgHash{formatdb}"; 
$seqstat = "${path}seqstat.jar";
$blastParser = "${path}blast_parser.pl";
...
###############################################################################
$ENV{CLASSPATH} = "{path}/$seqstat" if ($use_bootstrap);
...
$tmpdir = "/dev/shm/${fasta_seq_fileA}_${fasta_seq_fileB}";   # May be slightly (5%) faster using the RAM disk "/dev/shm".
system("rm -rf $tmpdir");
system("mkdir $tmpdir");
...
if ($run_blast){
    ...
    # formatdb is done apriori
    #print STDERR "Formatting BLAST databases\n";
    #system ("$formatdb -i $fasta_seq_fileA -l ${fasta_seq_fileA}.log");
    #system ("$formatdb -i $fasta_seq_fileB -l ${fasta_seq_fileB}.log") if (@ARGV >= 2);
    #system ("$formatdb -i $fasta_seq_fileC -l ${fasta_seq_fileC}.log") if ($use_outgroup);   
...
# and might not use the proper blast parser...
    ...
    do_blast ($fasta_seq_fileA, $fasta_seq_fileA, $A, $A, $blast_outputAA) if ( ! -f $blast_outputAA);

    if ((@ARGV >= 2) && ($fasta_seq_fileA ne $fasta_seq_fileB)) {
      do_blast ($fasta_seq_fileA, $fasta_seq_fileB, $B, $B, $blast_outputAB) if ( ! -f $blast_outputAB);
      do_blast ($fasta_seq_fileB, $fasta_seq_fileA, $A, $A, $blast_outputBA) if ( ! -f $blast_outputBA);
      do_blast ($fasta_seq_fileB, $fasta_seq_fileB, $B, $B, $blast_outputBB) if ( ! -f $blast_outputBB);
    }
...
sub bootstrap{
    ...
    open A, $file1 or die "bootstrap:: $file1 not found!\n";
    $id = 0;
    $print_this_seq = 0;
    $seq1 = "";
    $seq2 = "";

    $query_file = "${tmpdir}/${seq_id1}" . ".faq";                                                                              
    open Q, ">$query_file" or die "Cannot open $query_file for writing\n"; 
    ...
    close Q;
    ###
    open B, $file2 or die "bootstrap:: $file2 not found!\n";
    $db_file = "${tmpdir}/${seq_id2}" . ".fas";
    open DB, ">$db_file" or die "Cannot open $db_file for writing\n";
    $id = 0;
    $print_this_seq = 0;
    ...
    system "$blastall -F\"m S\" -i $query_file -z 5000000 -d $db_file -p blastp -M $matrix -m7 | $blastParser 0 -a > ${tmpdir}/${seq_id2}.faa";
    ...
    if (-s("${tmpdir}/${seq_id2}.faa")){

	system("java -jar $seqstat -m $matrix -n 1000 -i ${tmpdir}/${seq_id2}.faa > ${tmpdir}/${seq_id2}.bs"); # Can handle U, u

	if (-s("${tmpdir}/${seq_id2}.bs")){
	    open BS, "${tmpdir}/${seq_id2}.bs" or die "pac failed\n";
	    $_ = <BS>;
    ...
    unlink "${tmpdir}/${seq_id2}.fas", "${tmpdir}/${seq_id2}.faa", "${tmpdir}/${seq_id2}.bs", "${tmpdir}/${seq_id1}.faq"; 
    unlink "${tmpdir}/formatdb.log", "${tmpdir}/${seq_id2}.fas.psq", "${tmpdir}/${seq_id2.fas}.pin", "${tmpdir}/${seq_id2.fas}.phr";
